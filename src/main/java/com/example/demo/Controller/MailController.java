...
public class MailController {
	...
	public String sendMail(@ModelAttribute("jobapply") JobApply jobapply, Model model, HttpSession session) {
...
//		based on job applay id we are getting details of user 
		...
		ResponseEntity<JobApply> responseEntity = restTemplate
				.getForEntity("http://3.108.252.162:8080/JobNexaProject/api/v1/getjobApply/" + jobapply1, JobApply.class);
		...
		ResponseEntity<Test> responseEntity1 = restTemplate
				.getForEntity("http://3.108.252.162:8080/JobNexaProject/api/v1/gettestbyvacancyId/" + jobapp.getVacancies().getVacancyId(), Test.class);
		...
		selected.setVacancies(jobapp.getVacancies());
		...
		String url = "http://3.108.252.162:8080/JobNexaProject/api/v1/addSelectedCan";
		...
		body += "<p class=\"card-text\">We are thrilled to offer you the position of "+jobapp.getVacancies().getJobTitle()+" at "+jobapp.getVacancies().getCompany().getCompanyName()+".</p>";
		...
		body += "<p class=\"card-text\">Your role will involve "+jobapp.getVacancies().getDescription()+". We have a comprehensive onboarding process in place to ensure a smooth transition into your new role.</p>";
		body += "<p class=\"card-text\">You will be joining a dynamic team of professionals, including Manohar in various roles.</p>";
		body += "<p class=\"card-text\">At "+jobapp.getVacancies().getCompany().getCompanyName()+", we are committed to your professional growth and offer opportunities for career advancement through Career Development Programs.</p>";
		body += "<p class=\"card-text\">We are proud of our recent achievements, including "+jobapp.getVacancies().getCompany().getCompanyLevel()+", and look forward to achieving even more with your contribution.</p>";
		...
		body += "<p class=\"card-text\">If you have any questions, please refer to our FAQs <a href=\"" + jobapp.getVacancies().getCompany().getUrl() + "\">here</a> or feel free to reach out to our HR department at "+jobapp.getVacancies().getCompany().getUrl()+" or "+jobapp.getVacancies().getCompany().getMobile()+".</p>";
		...
		body += "    <img th:src=\"@{'http://3.108.252.162:8080/JobNexaProjectuploads/{logo}(logo=' + ${jobapp.getVacancy().getCompany().getLogo()} + ')}\" alt=\"Company Logo\" class=\"img-fluid\">\r\n"
				+ "";

		body += "<img src=\"http://3.108.252.162:8080/JobNexaProjectuploads/" + jobapp.getVacancies().getCompany().getLogo() + "\" alt=\"Company Logo\" class=\"img-fluid\">";
		body += "<p class=\"card-text mt-5\">Regards,<br>CEO/Management<br>"+jobapp.getVacancies().getCompany().getCompanyName()+"</p>";
		body += "</div>";
		body += "</div>";
		...
		String subject = "Congratulations on Your Job Offer at "+jobapp.getVacancies().getCompany().getCompanyName()+"";
	    ...
	    String url1 = "http://3.108.252.162:8080/JobNexaProject/sendMail";
		...
		return "redirect:/company/viewReports";
}
}